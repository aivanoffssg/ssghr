					...
					<div class="panel-body">
						...
						@if(count($results) > 0)
							<h4 class="text-right">
								...
								@else
									<a class="btn btn-success" style="color: #fff" href="{{ URL('downloadMyLeaveReport/?employee_id='.session('logged_session_data.employee_id').'&from_date='.dateConvertDBtoForm(date('Y-01-01')) .'&to_date='.dateConvertDBtoForm(date('Y-m-d')) )}}"><i class="fa fa-download fa-lg" aria-hidden="true"></i>  @lang('common.download') PDF</a>
								@endif
							</h4>
						@endif
                        @if(!empty($results))
